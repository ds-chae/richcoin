...
def delete_order(uuid, ):
    ...
    try:
        # Call API
        requests.delete(apiUrl + '/v1/order', params=param, headers=headers)
...
# Reusable session with retry/backoff and automatic pool cleanup
def make_session():
    retry = Retry(
        total=5,
        connect=5,
...
def safe_request(url, headers, timeout=(5, 20), max_attempts=6):
    global session
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            print(f"[warn] HTTPS error {e}, retry {attempt}/{max_attempts}")
            # Force socket close on retry
            if attempt == 1:
            ...
            # For 5xx will auto-retry; 4xx should break
            raise
...
def get_assets():
    ...
    assets = []
    try:
        response = safe_request(url, headers=get_access_header())
        rj = response.json()
        for r in rj:
            asset = OneAsset(r)
...
async def root_page(request: Request, ctoken: str = Cookie(None)):
    ...
    try:
        ...
        with open(index_path, 'rt', encoding='utf8') as htmlf:
            ...
            video_button = '<a href="https://sojucoin.com:8082/index.html" class="video-btn" style="background: #17a2b8; color: white; border: none; padding: 8px 16px; border-radius: 3px; cursor: pointer; font-size: 12px; font-weight: bold; text-decoration: none; margin-right: 10px; display: inline-block; min-width: 80px; min-height: 44px; text-align: center; line-height: 28px;">🎬 Video</a>'
